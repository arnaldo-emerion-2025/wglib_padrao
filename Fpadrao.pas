...
uses
  ...
  Wwdbcomb, dxExEdtr, dxEdLib, dxDBELib, dxDBGrid, dbTables, Db, Hnavspeed, DBGrids, DBCtrls;
...
type
  ...
  public
    ...
    procedure ModificaMascara(form: TForm; qtdZeros: Integer; tabela: string);
  end;
...
uses hNavigator, ManGDB, ManPri, Bbfuncao;
...
begin
  ...
  begin
    ...
    if not ((ActiveControl is TwwDbRichEdit) or (ActiveControl is TdxDBPickEdit) or
  ...
  begin
    ...
    if not ((ActiveControl is ThGrid) or (ActiveControl is TdxDBMemo)) then
    begin
  ...
  begin
    try
      ...
      begin
        ...
        if ((UpperCase(self.Components[i].Name) <> 'QUSQL')  and
            (UpperCase(self.Components[i].Name) <> 'QUSQL1') and
            (UpperCase(self.Components[i].Name) <> 'QUSQL2') and
            (UpperCase(self.Components[i].Name) <> 'QUSQL3')) then
        begin
          if self.Components[i] is TwwQuery then
          begin
            if trim(Twwquery(self.Components[i]).SQL.Text) <> '' then
          ...
          if ((UpperCase(self.Components[i].Name) <> 'QUSQL')  and
            ...
            begin
              if trim(TQuery(self.Components[i]).SQL.Text) <> '' then
...
procedure TfmPadrao.AplicPostError(DataSet: TDataSet; E: EDatabaseError; var Action: TDataAction);
begin
  showmessage(e.Message);
end;

procedure TfmPadrao.ModificaMascara(form: TForm; qtdZeros: Integer; tabela: string);
var
  I, J, tag: Integer;
  query: TwwQuery;
  decValor, decQtd: Integer;
  formato: string;
begin

  if qtdZeros <= 0 then
  begin
    decValor:= StrToIntDef(fmManGDB.BuscaSimples(tabela, 'DECIMAIS_VALOR', ' 1 = 1'), 4);
    decQtd  := StrToIntDef(fmManGDB.BuscaSimples(tabela, 'DECIMAIS_QTD'  , ' 1 = 1'), 4);
  end
  else
  begin
    decValor:= qtdZeros;
  ...
  begin
    ...
    if form.Components[I] is TwwQuery then
    begin

      for J := 0 to TwwQuery(form.Components[I]).FieldCount - 1 do
      begin
        query := TwwQuery(form.Components[I]);

        //TAG: 74 = 'VALORES'
        tag := TwwQuery(form.Components[I]).Fields[J].Tag;

        if (tag = 74) then
        begin
          formato                                   := '##,##0.' + Replicate('0', decValor);
          TFloatField(query.Fields[J]).DisplayFormat:= formato;
          TFloatField(query.Fields[J]).EditFormat   := formato;
        end;

        //TAG: 75 = 'QUANTIDADE'
        if (tag = 75) then
        begin
          formato                                   := '#,##0.' + Replicate('0', decQtd);
          TFloatField(query.Fields[J]).DisplayFormat:= formato;
          TFloatField(query.Fields[J]).EditFormat   := formato;
        end;
